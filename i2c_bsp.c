...
i2c_master_dev_handle_t pwr_expander_dev_handle = NULL;
...
void i2c_master_Init(void)
{
  ...
  // TCA9554 I/O expander: EXIO6 controls SYS_EN / battery power hold.
  dev_cfg.device_address = 0x20;
  ESP_ERROR_CHECK(i2c_master_bus_add_device(user_i2c_port0_handle, &dev_cfg, &pwr_expander_dev_handle));
...
#define TCA9554_SYS_EN_BIT (1U << 6)

uint8_t power_expander_set_hold(bool enable)
{
  if (pwr_expander_dev_handle == NULL) return ESP_ERR_INVALID_STATE;

  uint8_t cfg = 0x00; // all expander pins as outputs; required before driving EXIO6
  uint8_t ret = i2c_write_buff(pwr_expander_dev_handle, TCA9554_REG_CONFIG, &cfg, 1);
  if (ret != ESP_OK) return ret;

  uint8_t out = 0;
  ret = i2c_read_buff(pwr_expander_dev_handle, TCA9554_REG_OUTPUT, &out, 1);
  if (ret != ESP_OK) out = 0;

  if (enable) out |= TCA9554_SYS_EN_BIT;
  else out &= ~TCA9554_SYS_EN_BIT;

  return i2c_write_buff(pwr_expander_dev_handle, TCA9554_REG_OUTPUT, &out, 1);
}
